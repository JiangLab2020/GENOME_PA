...
#my $GeneAnnPath = "/public/home/chengj/GeneAnnotation";
...
my $PUGTA=$ENV{'PUGTA'};
my $PUGTA_lib=$ENV{'PUGTA_lib'};
#`cp $GeneAnnPath/Genome_augustus.pl $TargetDir/`;
#`cp $GeneAnnPath/Genome_augrice.pl $TargetDir/`;
#`cp $GeneAnnPath/Genome_glimmhmm.pl $TargetDir/`;
#`cp $GeneAnnPath/Uni_GeneAnn1.pl $TargetDir/`;
#`cp $GeneAnnPath/Uni_GeneAnn2.pl $TargetDir/`;
#`cp $GeneAnnPath/HMM_Pfam.pl $TargetDir/`;
#`cp $GeneAnnPath/Fasta_div.pl $TargetDir/`;
#`cp $GeneAnnPath/Adjust_div.pl $TargetDir/`;
#`cp $GeneAnnPath/LocRegionUni.pl $TargetDir/`;
#
`cp $PUGTA_lib/$Query_DB $TargetDir/Query_DB.pep`;
	...
	if ( $dots[$j] =~ /\.fasta/ and $size > 0 and $dots[$j] !~ /Trim_All.fasta/ ) {
		if ( $dots[$j] !~ /\_$Pfam\.fasta/ ) {
			if (! -e "$Auga_dir/$species\_augustus.pep") {
				`perl $PUGTA/Genome_augustus.pl $dots[$j] $CPU`;
				`perl $PUGTA/HMM_Pfam.pl $species\_augustus.pep $Pfam $CPU`;
			...
			if (! -e "$Augr_dir/$species\_augPU.pep") {
				`perl $PUGTA/Genome_augPUGT.pl $dots[$j] $CPU`;
				`perl $PUGTA/HMM_Pfam.pl $species\_augPU.pep $Pfam $CPU`;
			...
			if (! -e "$Glm_dir/$species\_glimmerHMM.pep") {
				`perl $PUGTA/Genome_glimmhmm.pl $dots[$j]`;
				`perl $PUGTA/HMM_Pfam.pl $species\_glimmerHMM.pep $Pfam $CPU`;
		...
		if ( !(-e "$Merge_dir/$species\_uni_DB.pep") ) {
			...
			if ( @Files ) {
				...
				`perl $PUGTA/Uni_GeneAnn1.pl $file`;
...
if ( !(-e "All_uni_DB_adj_div.pep") ) {
	...
	`perl $PUGTA/Fasta_div.pl All_Methods.pep Query_DB.pep $CPU`;
	`cat Merge_dir/*_uni_DB.pep >All_uni_DB.pep`;

	`perl $PUGTA/Adjust_div.pl`;
	`perl $PUGTA/Uni_GeneAnn2.pl All_uni_DB_adj_div.txt All_uni_DB_adj.pep 0.2`;
}
