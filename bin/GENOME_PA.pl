...
close OUTPUT;
`perl $PUGTA/1_Blast_GenomeUGT.pl $Blast_MapDIR $CPU $Genome_dir`;

print '##############################    ',"2: ","Three methods Annotate homo-Genomes\n";
#chdir("$Path/Genomedir");

`perl $PUGTA/Genome_Annoation.pl $Genome_dir $CPU $Pfam $Query_DB`;

print '##############################    ',"3: ","Filtering good proteins in homo-Genomes\n";

`perl $PUGTA/GeneAnn_TrimRightSeq.pl $Genome_dir`;
...
`perl $PUGTA/GeneAnn_BlastGene.pl Trim_All.fasta $PUGTA_lib/$Query_DB 0 $CPU`;
...
`perl $PUGTA/Genome_Annoation.pl $TargetDirBlast $CPU $Pfam $Query_DB`;
...
`perl $PUGTA/Uni_GeneAnn3.pl All_uni_DB_adj_div_Com.txt All_uni_DB_adj_div_Com.pep 0.2`;
`cp All_uni_DB_adj_div_Com_Final.pep $ResultDir/Genome_PA.pep`;
`cp All_uni_DB_adj_div_Com_Final.txt $ResultDir/Genome_PA.txt`;
