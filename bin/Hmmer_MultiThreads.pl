...
my $PfamPath = "$PUGTA_lib\/Pfam-A.hmm";

my $PF_ID   = 0;                             ####  默认是注释所有家族, 或者指定家族PF_ID.
if ( $ARGV[2] ) {
	$PF_ID  = $ARGV[2];
	$PfamPath = "$PUGTA_lib\/HMM/$PF_ID\.hmm";
	if ( !(-e "$PUGTA_lib\/HMM/$PF_ID\.hmm.h3f") ) {
		if ( -e "$PUGTA_lib\/HMM/$PF_ID\.hmm.h3i" ) {
			unlink("$PUGTA_lib\/HMM/$PF_ID\.hmm.h3i");
		}
		`hmmpress $PfamPath`;
	}	
}
