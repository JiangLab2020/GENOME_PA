...
my $PUGTA=$ENV{'PUGTA'};
#my @AnnFiles = qw(660_660_augustus_UGT.fasta 660_660_glimmerHMM_Adj_UGT.fasta);
...
`perl $PUGTA/Tab_Rank.pl GeneLoc.txt GeneLoc_Sort.txt 2 3 4`;
`perl $PUGTA/LocRegionUni.pl GeneLoc_Sort.txt`;
...
if ( !(-e "GeneLoc_Sort_uni_DB.txt") ) {
	`perl $PUGTA/Blast_MultiThreads.pl $Query_DB GeneLoc_Sort_uni.pep GeneLoc_Sort_uni_DB.txt 1 $BlastNum $CPU_Num`;
}
...
foreach my $key (keys %AList) {
	...
	if ( $AlignNum > 1 ) {	
		...
		`perl $PUGTA/Fasta_DelLargeDiv.pl Tem_Tem.fas Tem_Tem.txt 0`;
